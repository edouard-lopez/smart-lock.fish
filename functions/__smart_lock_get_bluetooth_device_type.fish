function __smart_lock_get_bluetooth_device_type \
    ...
    if not echo $bluetooth_device | grep --quiet "Connected: yes"
        return 1
    end

    if echo $bluetooth_device | grep --quiet 'Icon: phone'
        echo phone
    else if echo $bluetooth_device | grep --quiet 'Icon: input-mouse'
        echo mouse
    else
        echo other
    end
